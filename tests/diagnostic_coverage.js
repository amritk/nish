 ...
 *      fails twice over: the `.err` no longer matches, and the registry
 *      fragment the code is keyed on is no longer in the message either.
 ...
 *      A new diagnostic therefore arrives with a case or with a sentence saying
 *      why it cannot have one — which is the property the gate wants, and the
 *      one that quietly stops holding otherwise.
 ...
 * `tests/wordings/parser_refusals.txt`, and its `.err` pins what the compiler
 * says instead: the first syntax error's message. The wording stage0 used to
 * print for it went with stage0, and the summary counts these cases apart.
 *
 ...
 * a reason". Asked of stage1 it once answered with 80 findings: codes that
 * programs do provoke, but only under stage0, because stage1 answers those
 * programs in its parser. While stage0 lived, `tests/wordings/stage0_only.txt`
 * joined each such code to the programs that provoked it; at R6 it emptied into
 * `unreachable.txt` as retired codes, because nothing prints those wordings any
 * more.
 *
  ...
  cases.forEach((c, i) => {
    ...
    if (hit === undefined) {
      ...
      if (refusedByParser(result) && refusals.has(c.stem)) {
        // The `.err` of a declared refusal pins what the compiler says
