...
declare noalias noundef nonnull align 8 i8* @sts_read_file(i8* noundef nonnull readonly align 8 nocapture) #1
...
declare void @sts_append_file(i8* noundef nonnull readonly align 8 nocapture, i8* noundef nonnull readonly align 8 nocapture) #1
...
entry:
  ...
  call void @sts_append_file(i8* %1, i8* bitcast ({ i64, [7 x i8] }* @.str.2 to i8*))
  %2 = load i8*, i8** %path.addr, align 8
  %3 = call i8* @sts_read_file(i8* %2)
