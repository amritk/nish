...
for.cond:
  ...
  %3 = fcmp olt double %2, 0x4020000000000000
...
walk.body:
  ...
  %21 = load double, double* %k.addr, align 8
  %22 = fcmp oeq double %21, 0x3FF0000000000000
  br i1 %22, label %if.then, label %if.end

if.then:
  %23 = load %struct.Map$f64$f64*, %struct.Map$f64$f64** %m.addr, align 8
  %24 = call i1 @nish.Map$f64$f64.delete(%struct.Map$f64$f64* %23, double 0x4008000000000000)
  %25 = load %struct.Map$f64$f64*, %struct.Map$f64$f64** %m.addr, align 8
  %26 = call i1 @nish.Map$f64$f64.delete(%struct.Map$f64$f64* %25, double 0x3FF0000000000000)
  %27 = load %struct.Map$f64$f64*, %struct.Map$f64$f64** %m.addr, align 8
  %28 = call i1 @nish.Map$f64$f64.delete(%struct.Map$f64$f64* %27, double 0x0000000000000000)
  br label %if.end

if.end:
  %29 = load double, double* %k.addr, align 8
  %30 = fcmp oeq double %29, 0x4010000000000000
  br i1 %30, label %if.then.1, label %if.end.1

if.then.1:
  %31 = load %struct.Map$f64$f64*, %struct.Map$f64$f64** %m.addr, align 8
  %32 = call i1 @nish.Map$f64$f64.delete(%struct.Map$f64$f64* %31, double 0x401C000000000000)
  br label %if.end.1
...
walk.inc:
  %39 = add i32 %12, 1
  %40 = call i32 @nish.Map$f64$f64.walkNext(%struct.Map$f64$f64* %10, i32 %39)
  store i32 %40, i32* %walk.idx, align 4
  br label %walk.cond

walk.end:
  call void @nish.Map$f64$f64.walkClose(%struct.Map$f64$f64* %10)
  %41 = load %struct.Map$f64$f64*, %struct.Map$f64$f64** %m.addr, align 8
  %42 = getelementptr inbounds %struct.Map$f64$f64, %struct.Map$f64$f64* %41, i32 0, i32 0
  %43 = load double, double* %42, align 8, !tbaa !6
  %44 = call i8* @nish_str_from_f64(double %43)
  %45 = call i8* @nish_str_concat(i8* bitcast ({ i64, [6 x i8] }* @.str.0 to i8*), i8* %44)
