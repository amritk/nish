// `perf_bounds_toi32` under `--number-mode f64`, where `w.length` is an `f64`
// and `toI32` lowers to a saturating `fptosi`: still exact, so still proven.
//
...
// `perf_bounds_toi32` is this program in the default i32 mode.
