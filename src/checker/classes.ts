 ...
 * semantics, no inheritance, no vtable.
 *
 ...
 *     signatures, `extends`, parameter properties, or class expressions;
 *     `public`/`private`/`protected` are accepted and ignored;
 *   - `readonly` fields are assignable only as `this.f = ...` in the class's
 *     own constructor;
...
import { FieldInfo, FunctionSig, LocalVar, Param, StructInfo } from "./program";
...
/** True when `t` is a class type that lists interface `iface` in its `implements` clause. */
export function implementsInterface(ctx: CheckContext, t: StaticType, iface: StaticType): boolean {
  ...
  return !!cls && !!target && cls.kind === "class" && target.kind === "interface" && cls.implements.includes(iface.name);
}
...
export function declareStruct(ctx: CheckContext, decl: ts.ClassDeclaration | ts.InterfaceDeclaration): StructInfo {
  ...
  for (const clause of decl.heritageClauses ?? []) {
    if (clause.token === ts.SyntaxKind.ExtendsKeyword) {
      throw ctx.error(
        kind === "class"
          ? "Class inheritance (`extends`) is not supported yet (WP2b); use a field of the base type"
...
/** Resolve fields, layout, methods and the constructor of a registered struct. */
...
function checkDefiniteAssignment(ctx: CheckContext, cls: StructInfo): void {
  ...
  const body = (cls.ctor.decl as ts.ConstructorDeclaration).body!;
  const result = new DefiniteAssignment(ctx, cls).statements(body.statements, initialised);
...
function calleeSignature(ctx: CheckContext, call: ts.CallExpression | ts.NewExpression): FunctionSig | undefined {
  if (ts.isNewExpression(call)) {
    ...
    return ctx.program.structs.get(call.expression.text)?.ctor;
  }
  ...
  if (ts.isPropertyAccessExpression(call.expression)) {
    ...
    if (receiver?.kind !== "struct") return undefined;
    return ctx.program.structs.get(receiver.name)?.methods.get(call.expression.name.text);
  }
  return undefined;
}
 ...
 * report the interface type so the existing `sameType` checks accept it.
 */
...
newCheckers["*"] = (ctx, expr, scope) => {
  ...
  if (info.ctor) {
    checkMethodArguments(ctx, expr, info.ctor, args, `new ${name}`, scope);
