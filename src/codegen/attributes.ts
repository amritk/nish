...
/**
 * Gather per-function facts, then propagate over the call graph to a
 * fixpoint: a function is as impure as the most impure thing it calls, and
 * it is willreturn only if everything it calls is.
 */
...
export function isCountedLoop(program: CheckedProgram, loop: ts.IterationStatement): boolean {
  ...
  if (!iv || iv.type.kind !== "i32") return false;
