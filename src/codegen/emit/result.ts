 ...
 *
 *   Ok(v)            `%0 = call i8* @nish_alloc_struct(i64 <size>)`
 *                    `%1 = bitcast i8* %0 to %struct.nish_result.<T>.<E>*`
...
const allocateIn = (ctx: EmitContext, layout: ResultLayout, stack: boolean): string => {
  ...
  const raw = ctx.fn.emitValue(`call i8* ${ctx.useRuntime("nish_alloc_struct")}(i64 ${layout.size})`);
  ...
  if (ts.isCallExpression(node)) {
    ...
    if (callee !== undefined) {
      if (resultByValue(callee.returnType)) {
        if (!facts.stackSites.has(node)) facts.callees.add("nish_alloc_struct");
        facts.effect = "write";
      }
      return;
    }
    if (ts.isPropertyAccessExpression(node.expression)) {
      ...
      if (method === "orReturn") {
        ...
        facts.callees.add("nish_write");
        facts.callees.add("nish_exit");
      }
      return;
    }
