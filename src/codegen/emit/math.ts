 ...
 *   Math.random()    call double @nish_random()   (write effect: RNG state)
 ...
 *   parseFloat(s)          call double @nish_parse_number(i8* s, i32 0)
 *   Number(s)              call double @nish_parse_number(i8* s, i32 1)   (string)
 *   Number(x)              sitofp / `uitofp i1` / nothing                (i32, i64, boolean, f64)
 *   parseInt(s)            @nish_parse_number(s, i32 2) then @llvm.fptosi.sat.i32.f64
 *
...
const mathRandom: BuiltinCall = {
  emit: (ctx) => ctx.fn.emitValue(`call double ${ctx.useRuntime("nish_random")}()`),
  callees: () => ["nish_random"],
};
...
const PARSE_RUNTIME = "nish_parse_number";
