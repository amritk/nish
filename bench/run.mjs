#!/usr/bin/env node
// StaticTS benchmark runner (WP9). Builds every benchmark in bench/ for
// StaticTS, C and Rust, checks that all of them print the same checksum, times
...
// that `%.17g`, Rust's `{}` and StaticTS's JavaScript-style formatting of the
...
/**
 * `mode` is the `--number-mode` the StaticTS source needs; `integer` marks the
...
function build(bench) {
  ...
  const sts = (id, label, extra, profile, timed) => {
    ...
    variants.push({ id, label, exe, timed, cmd: `statictsc ${args.join(" ")}` });
  };
  sts("sts", "StaticTS", [], "speed", true);
  if (bench.integer) sts("sts-nsw", "StaticTS --nsw", ["--nsw"], "speed", true);
  sts("sts-size", "StaticTS (size profile)", [], "size", false);
...
lines.push(`| statictsc | ${version("node", [cli, "--version"])} (commit ${commit}) |`);
...
lines.push(`| Benchmark | ${columns.map(([, l]) => l).join(" | ")} | StaticTS / Rust |`);
...
lines.push("", "The last column divides the StaticTS minimum by the Rust `-O3` minimum: 1.00x is parity, above 1.10x misses the WP9 target. Rust native is `-C target-cpu=native`; C is plain `-O3` without `-march`.", "");

lines.push("## Binary size (bytes, stripped)", "");
lines.push("| Benchmark | StaticTS speed | StaticTS size | C `-O3` | Rust `-O3` |", "| --- | ---: | ---: | ---: | ---: |");
for (const r of results) {
  const b = (id) => r.variants.find((v) => v.id === id)?.bytes.toLocaleString("en-US") ?? "";
  lines.push(`| ${r.bench.name} | ${b("sts")} | ${b("sts-size")} | ${b("c")} | ${b("rust")} |`);
}
lines.push("", "StaticTS binaries link `runtime/runtime.c` statically and glibc dynamically; the Rust binaries carry `std` statically (`panic=abort`, `strip=symbols`).", "");
...
lines.push("Rust flags: `-C opt-level=3 -C panic=abort -C codegen-units=1 -C strip=symbols`, plus `-C target-cpu=native` for the native column. `statictsc --link` runs `scripts/build.sh --profile speed` (`clang -O3 -flto` with section GC and stripping; see the README) over the module and `runtime/runtime.c`.", "");
