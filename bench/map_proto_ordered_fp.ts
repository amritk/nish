// WP32 layout prototype 2 of 4: insertion-ordered, with an `i64` slot that
// packs the key's full 32-bit hash above the entry index plus one
// (docs/wp32-map.md §2). A probe reads only the slot array until the hash in
// a slot equals the key's, so a miss, and nearly every bucket that belongs to
// another key, costs one load; the entry list is touched once, on the match.
// Each entry keeps its hash too, so growth and compaction re-file the slots
// from `hashes` and never hash a key again.
//
...
/** A deleted entry's bucket: hash field 0, which no key has, and index field all ones. */
const TOMB: i64 = 4294967295;
...
const slotOf = (h: u32, index: i32): i64 => (toI64(h) << 32) | toI64(index + 1);

class StrMap {
  /** Bucket -> the full hash in the high 32 bits, entry index plus one in the low 32. */
  slots: i64[];
  mask: i32 = 7;
  keys: string[];
  values: i32[];
  ...
  constructor() {
    this.slots = new Array<i64>(INITIAL_SLOTS);
    this.keys = [];
    this.values = [];
    this.hashes = [];
  }
   ...
   * take. The slot carries the whole hash, so the entry list is read only
   * when the hashes are equal, and then the key compare almost always matches.
   */
  probe(key: string, h: u32): i32 {
    ...
    while (b >= 0 && b < n) {
      ...
      // A deleted bucket's hash field is 0, which no key's hash is.
      if (toU32(s >>> 32) === h) {
        const at = toI32(toU32(s)) - 1;
