...
//   amritc examples/argv.ts --link build/argv && ./build/argv 3 4 five
...
export function main(): number {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    // biome-ignore lint/suspicious/noSelfCompare: NaN is the only value that is not itself; AmritScript has no Number.isNaN
    const parsed = n === n ? `${n}` : "not a number";
    console.log(`  ${i}: ${args[i]} -> ${parsed}`);
    // biome-ignore lint/correctness/useParseIntRadix: AmritScript parseInt is base 10 only and takes one argument
    sum += parseInt(args[i]);
  }
