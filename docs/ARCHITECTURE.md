...
| `readonly` (struct/array param) | `!writesThrough` after the fixpoint | No store through the pointer, no escape, only passed to `readonly` parameters. Array element stores through nested indexing count as writes, conservatively. An element store into an array field stored inside the object (`p.f[i] = v`, `self/inline_arrays.ts`) is a store to the object's own bytes, so it counts as a write through `p` (`classifyMemberUse`); with the pointer layout the same store is only a read of `p` (`tests/cases/cls_inline_array`: `Board.mark` is not `readonly`, `Board.sum` is). |
