...
program can read Nish objects through a matching `struct` (see the layout
test below). `nish_alloc_struct` rounds the allocation up to 8 bytes and
...
- `new Point(3, 4)`: `call i8* @nish_alloc_struct(i64 <sizeof>)`, a `bitcast`
...
- **C header.** `--emit-header` declares every class and interface as a
  ...
  `NISH_SYMBOL("Class.method")` (docs/wp8-interop.md).
...
- `tests/layout/structs.ts` + `structs.c`: the runner reads each class's
  `nish_alloc_struct(i64 N)` from the IR and compares it with the
