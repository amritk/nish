...
`nish_str_concat` keeps the attribute).
...
- **Number formatting.** `number` holes and `console.log(number)` use
  `nish_str_from_i32` / `nish_str_from_i64` (exact) or `nish_str_from_f64`,
...
entry:
  ...
  %1 = call i8* @nish_str_concat(i8* %0, i8* bitcast ({ i64, [2 x i8] }* @.str.2 to i8*))
...
`nish_str_concat` allocates in the arena (effect `write`), so `join` is
...
Comparison is by content (`nish_str_eq` short-circuits on pointer equality).
...
converted to strings (`nish_str_from_i32` / `nish_str_from_f64`, `select` for
booleans, identity for strings), and the parts are joined left to right with
`nish_str_concat`. Chaining was chosen over an `nish_str_concat_n` runtime
...
| `a + b` | calls `nish_str_concat` (write) |
| `a === b`, `a !== b` | calls `nish_str_eq` (read) |
| `s.length` | `readsMemory` (read) |
| template | `nish_str_concat` when more than one part; `nish_str_from_i32/f64` per numeric hole |
| `console.log(x)` | `nish_print` (write) plus the conversion for `x` |
