...
| | Milestone | Done when |
| --- | --- | --- |
| **R1** | Parity | **red when last measured, and a row that does not carry a date is not a measurement.** The measurement, in the mode's own words: `parity: 13260 runs over 884 programs (2100.5 s); 14 undeclared difference(s), 2678 declared` — 2026-09-18, `ubuntu-latest` with LLVM 18, [run 35337709303](https://github.com/amritk/nish/actions/runs/35337709303) at `2688ea0`, which is `main` at `4e8964f` plus a comment. All **14** rows are one case, `tests/cases/reject_generic_expanding_field`, under fourteen variations: stage1 asks for a contextual `T \| null` annotation at 13:22 where stage0 says ``Unknown field `inner` on class `Nest$i32` `` at 13:12 — two different *second* complaints about one line, both of them cascades from the instantiation the termination rule already refused, and WP18's rather than this package's. The other class §A7 names is **closed**: the 4 rows where stage1 exited 70 under `-g` on a `CPtr` program went with #90. §4's builtins landed in both compilers, the seven rows of §2A closed, §A2's five closed (four fixed, the fifth re-read as §A3's recovery class), and §A3's five classes are closed or declared. The mode has been recorded green three times and found red on the next run **four** times — §A5 and §A7 — and the corpus is the reason every time: 764 programs, then 815, then 868, then 884. **Do not quote this row without re-running the mode**, and read §A7 for why a number here goes stale in about a day. What has changed is not how long a number keeps but where a red one lands: the nightly's `record` job fired for the first time on this run and opened #93 with the difference table in it, and `ci.yml`'s `parity-changed` job runs the corpus half on every pull request over the programs that pull request touched — so the next difference of this shape is red on the pull request that writes it rather than in a nightly four nights later. That bound is itself checked now: `changedPrograms` maps a diff's files onto the programs that own them by **ownership, not substring** — the `<stem>.` prefix and the `tests/link/<name>/` slash — and sixteen checks in `tests/run.js` pin it on every `npm test`, because the rule could be broken (a bare substring match selects `add` for a diff naming `add_plain`) with the whole suite still reporting `1960 passed, 0 failed`. A **deletion** is reported rather than compared, and on purpose: a program the diff removed has no source for either compiler and cannot carry a reintroduced difference, so it is not selected — but it does shrink the corpus, so `removedPrograms` names each one and the job summary says how many instead of answering that the pull request touched nothing |
