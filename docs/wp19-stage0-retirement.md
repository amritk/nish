...
§A5 lists three ways a closed gate reopens in silence and §A7 adds a fourth —
the corpus growing under a number somebody wrote down. **This is a fifth, and
it is the one the previous four could not have found: the cross product is only
as wide as `VARIATIONS`, and `--json` was not in it.** The compiler's one
...
opened for the red run of 2026-09-18 and closed by a green full run. **State,
2026-09-19**, measured on `ae6b45b` with #117's branch on top — the tree that
merged as `862c7cc`:

```
parity: 14432 runs over 902 programs (3396.2 s); 0 undeclared difference(s), 2798 declared
```

exit 0, from `node tests/run.js --parity`. The seconds are a shared four-core
box and measure the neighbours as much as the compilers (§A7); the counts are
the gate. No full-corpus run has been taken on merged `main` since — that is
the nightly's job, and §5a says so where the number is quoted.
...
2. the **ld64 fixed point** — the `stage3 == stage2` family above. **This has
   been run on a mac and root-caused, on a branch that is not merged (#114):
   read the next paragraph as a claim about that branch, not about `main`.**
   The failure was the *harness's* and not the toolchain's. `bootstrap.sh`
   ...
   **What is measured, stated exactly**: the UUID is stable across two links to
   ...
   or attributed it to ld64's debug map, is stale as of that branch.
...
narrowing** — on a branch that is escalated and unmerged (#114), so this
paragraph describes that branch and not `main`. The differing bytes were
...
one, and the three rows can be attached rather than held (#92). Until that
branch merges and a release carries those assets, `attachedSince` stands where
it is and G3's macOS `bootstrap` row does not appear — a release already
published cannot grow an asset, which is the distinction this row has been
...
remembering — on a branch that is escalated and unmerged (#116), so this
paragraph is a claim about that branch and not about `main`.** A tag that
...
only order in which a tag cannot outrun the thing it certifies — and 19 checks
drive every arm of that script against a git stand-in, which is the same move
G3's `seed-matrix.sh` made for the same reason: both times this logic was got
wrong, it was inline shell in a workflow that nothing could run.
...
| --- | --- | --- |
| **G1** | parity over the corpus, every flag variation | `parity: 14432 runs over 902 programs (3396.2 s); 0 undeclared difference(s), 2798 declared` — `node tests/run.js --parity`, 2026-09-19, on `ae6b45b` plus the branch of #117. **Not** a run on merged `main`: see the note below this table |
...
| **G6** | the re-verification procedure | written in §G6 above, four commands, unchanged |
...
**Read the G1 row's attribution, because this document's own standard is that a
row carries its date and its command — not that whoever wrote it did the
measuring.** That number was taken on `ae6b45b` with #117's branch on top,
which is the tree the change was verified on and the tree it then merged to as
`862c7cc`. **A fresh full-corpus `--parity` on merged `main` has not been taken
by this stage.** The corpus half is forty-odd minutes and this stage did not
block on one; what produces `main`'s own number is the nightly `Parity`
workflow, on the default branch, which is exactly the arrangement §A5 argues
for — a gate whose freshness does not depend on somebody remembering. What
`862c7cc` itself carries is [CI run 553](https://github.com/amritk/nish/actions/runs/35454091607),
a green full `npm test`, which includes the flag-set half of G1, the bootstrap
equalities and the diagnostic-coverage tool over both compilers. The corpus
half is the one measured a commit earlier.
...
2. **Merging the darwin fixed point, and a release that carries the seed**
   (G3, G5). The measurement itself is done: root-caused on a mac and remedied,
   on an escalated but unmerged branch (#114 — see R2 and §G5). What is owed is
   the merge, and then a release that attaches the darwin assets, after which
   G3's macOS `bootstrap` row appears on its own from `seed-targets.json`. A
   release already published cannot grow an asset, so this one is sequenced
