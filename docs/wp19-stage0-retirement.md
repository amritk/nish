...
exit 0 — **over 909 of the 986 programs the corpus holds now.** §A9 put
...
grew and a gate that was widened. **What is owed is the full number on the new
corpus**, and it is owed rather than guessed at: `node tests/run.js --parity`
on the head this lands on, or the nightly `Parity` workflow the morning after.
It was attempted twice here and neither run may be quoted — the first was
killed when `main` moved under it, the second was SIGTERMed by a neighbour on a
shared four-core box at load 13, and a run that prints no summary line is a run
that says nothing (the rule this document's own §A7 and `.claude/testing.md`
both state). The seconds are a shared four-core box and measure the neighbours as
