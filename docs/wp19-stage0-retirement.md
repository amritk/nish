...
remembering — on #116, which is green and pending merge but **not merged as of
2026-09-19 17:20 UTC**, so this paragraph is a claim about that pull request
and not yet about `main`.** A tag that
...
of that script is driven against a git stand-in, which is the same move G3's
`seed-matrix.sh` made for the same reason: both times this logic was got wrong,
it was inline shell in a workflow that nothing could run. **Read the check count
from a run rather than from here** — this document has twice been wrong about a
count it copied instead of re-deriving (§A7).
...
| **R5** | Provenance | **the procedure is written and the tag is automated on #116, which is green and pending merge rather than merged.** §G6 has the four-command re-verification, unchanged. What was outstanding is that the tag was cut "at release time", meaning by whoever remembered: a provenance record whose only trigger is memory, for a property that cannot be re-established once lost (§2D). On #116 `release.yml` cuts it — `.github/ddc-tag.sh` decides the tag, the `release` job `needs: ddc`, and it is cut after the jobs that prove the equalities and before `gh release create`, so the tag can never certify a commit the proof did not pass; every arm of the script is driven against a git stand-in rather than being shell nothing runs. **Until #116 merges, `main` still cuts this tag by hand** — not merged as of 2026-09-19 17:20 UTC — and this row is that pull request's claim rather than the tree's. It is a row that will be right shortly and is not right yet, which is the distinction the whole of §A7 is about |
