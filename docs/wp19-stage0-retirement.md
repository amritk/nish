...
> [`AGENTS.md`](../AGENTS.md) are the rules for the one compiler that is left.
...
```
parity: 14544 runs over 909 programs (2663.3 s); 0 undeclared difference(s), 2808 declared   2026-09-22, on 3927242, over a corpus that did not hold these 71
parity: 1136 runs over 71 programs (346.6 s);   0 undeclared difference(s),  142 declared   2026-09-22, the 71 under all sixteen variations, once they were in it
```
...
**And the gap reached the pull-request gate as a *skip*.** `ci.yml`'s
`parity-changed` job runs the corpus half over the programs a diff touches, and
`changedPrograms` maps a path onto a program only through `corpus()` — so a
pull request whose whole subject is `tests/differential/**` touches no corpus
program, the job is *skipped*, and the summary says nothing was compared
because nothing needed to be. #141, which edits that directory's harness and
nothing else, shows exactly that. A skip there is the honest answer to "did
this diff touch a corpus program?" and the wrong answer to "was this diff's
subject compared", and the two questions were the same question for as long as
the directory was outside `CORPUS_DIRS`. It is inside now, so a diff that
touches one of the 71 selects it.

**The widened surface says the same thing, which is worth having because it
could have said otherwise.** Compiled under 14 of `parity.js`'s sixteen
variations — all but `--emit-ast`, whose output differs by design, and skipping
a flag a program's own `.args` already sets — 974 runs over the 71 programs
give 949 agreements and 25 disagreements, over the same **two** distinct
programs (`conversions_roundtrip` 12 variations, `str_template` 13). The one
variation where both agree is `--number-mode f64`, which is the diagnosis: in
f64 mode the literal's default type *is* `f64`, so the context it was not
reading was the context it did not need.
...
`node tests/run.js --parity` on merged `main` at `1e95aac`:

```
parity: 15776 runs over 986 programs (2938.5 s); 0 undeclared difference(s), 2961 declared
```

exit 0, **over the whole corpus as it now stands**, which is the number the
paragraphs below were written owing. The figure it supersedes — `14544 runs
over 909 programs … 2808 declared`, on `3927242` — was measured before §A9 put
`tests/differential/corpus` and the directory programs into `CORPUS_DIRS`, so
the cross product is 986 programs and 15,776 runs, and the number above was
measured before that. Of the 77 programs between the two counts, **74 are this
widening** — the 71, `examples/multi/main.ts`, and the two cases §A9's checker
fix ships with — and the rest arrived with WP30 and the launcher change. The 74
were measured under all sixteen variations and are green:
`1136 runs over 71 programs … 0 undeclared, 142 declared` for the differential
corpus, `64 runs over 4 programs … 0 undeclared` for the `multi` selection, and
16 runs each for the two cases §A9's checker fix ships with. Every one of those
142 declarations is a class that already existed — 71 × `--emit-checked stdout`
and 71 × `--emit-ast stdout` — which is the difference between a corpus that
grew and a gate that was widened. **That owed number is paid, and it took three
attempts of which only the third may be quoted.** The first was killed when
`main` moved under it; the second was SIGTERMed by a neighbour on a shared
four-core box at load 13; the third ran on an idle box (load 0.24, four cores)
with `npm ci` and the build completed before it started, because a run that
prints no summary line is a run that says nothing — the rule this document's
own §A7 and `.claude/testing.md` both state.

**The declared count moved 2808 → 2961, and that delta is deliberately not
decomposed here rather than decomposed badly.** 142 of the 153 are §A9's 74
programs meeting declaration classes that already existed; the remaining 11
belong to the `multi` selection and to the programs WP30 brought, and nobody
has attributed them one by one. A decomposition has to read the mode's **own**
program set rather than the corpus module's: `parity.js` counts 986 where
`programs()` and `linkPrograms()` enumerate 538, because the mode compares
refusals too, and an attempt to decompose it through the corpus module is how
this paragraph would have said something false. The seconds are a shared four-core box and measure the neighbours as
...
| --- | --- | --- |
| **G1** | parity over the corpus, every flag variation | **Re-derived over the whole corpus, 2026-09-22.** `parity: 15776 runs over 986 programs (2938.5 s); 0 undeclared difference(s), 2961 declared` — `node tests/run.js --parity`, exit 0, on merged `main` at `1e95aac`, on an idle box with the build finished before the run began. **This is the figure this row was owed.** What it supersedes, `14544 runs over 909 programs … 2808 declared` at `3927242`, was measured before §A9 found `tests/differential/corpus` outside `CORPUS_DIRS` and put it in, so it covered 909 of the 986 programs and none of those 71. The **74** this widening adds — the 71, `examples/multi/main.ts` and the two cases the checker fix ships with, the remaining three of the 77 having arrived with WP30 — are green under all sixteen variations — `parity: 1136 runs over 71 programs (346.6 s); 0 undeclared difference(s), 142 declared` for the differential corpus, and `64 runs over 4 programs` / 16 runs / 16 runs for the rest, all 0 undeclared, with every declaration in a class that already existed. The full figure is stated at the head of this row rather than owed; it took three attempts, and the two that printed no summary line are quoted nowhere. It replaces `14432 runs over 902 programs … 2798 declared` at `862c7cc`, and the deltas **decompose rather than needing to be trusted**: seven entry programs arrived between the two runs — `io_realpath`, `reject_realpath_arity`, `reject_realpath_type`, `builtin_realpath`, `reject_cls_field_init_type_twice`, `reject_std_escaping_module` and `module_stem_clash/main.ts`, the clash case's two imported `text.ts` being modules rather than programs — and seven programs across the sixteen variations is exactly the 112 new runs. The ten new declared rows are those programs meeting declaration classes that already existed: no `DECLARED` entry names any of them, which is the difference between a corpus that grew and a gate that was widened |
