...
**State, read on the tree as of 2026-09-19 (`7ff7ce1`): the workflow builds
all four, one of them ships today, and the package is unchanged with no registry
name to install it under.** The checks behind this row are
...
`node tests/run.js ddc-tag` prints — **19, on 2026-09-19, on the merged tree at
`7ff7ce1`** — and it is derived here rather than copied
