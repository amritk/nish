...
| `u8`, `u16`, `u32` | `i8`, `i16`, `i32` | `uint8_t`, `uint16_t`, `uint32_t` | skipped | `number`, put back in range by the loader: `& 0xff` / `& 0xffff` on a narrow argument, and `& 0xff` / `& 0xffff` / `>>> 0` on every result |
| `u64` | `i64` | `uint64_t` | skipped | `bigint`, read back with `BigInt.asUintN(64, x)` |
| `f32` | `float` | `float` | skipped | `number`, untouched: the call rounds an argument to f32 and every f32 is exact in the double a result arrives in |
...
| `tests/self/interop_unsigned.ts` | The unsigned widths as bare parameters and results: the oracle's fixture for the loader's masks, and the module `tests/run.js` builds to wasm and calls. |
...
- Classes, `T | null`, `string[]`, `boolean[]` and nested arrays across
  ...
  than silently omitting them, naming the argument and the type that stopped
  each one.
- The unsigned widths and `f32` through the **N-API** shim: `SCALAR_READERS`
  and `scalarBox` in `src/interop/napi.ts` have a row per width and no
  unsigned or `f32` row, so a function mentioning one is skipped with the
  usual note. The C header and the wasm bridge carry all of them. Adding the
  rows is a `napi_get_value_uint32` / `napi_get_value_bigint_uint64` /
  `napi_create_uint32` table and the same range decision the wasm loader made
  above.
