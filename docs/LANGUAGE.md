...
This is the normative reference for Nish, the subset of TypeScript that
`nish` compiles to LLVM IR. It describes what the compiler accepts today,
...
- **`null`** is a value only where a `T | null` type is expected (see
  ...
  **`undefined`** is forbidden (`` `undefined` is forbidden in Nish; use `null` with a `T | null` type ``,
...
- **Printing gives JavaScript's digits for the float's value.**
  ...
  `nish_str_from_f64`, so `const tenth: f32 = 0.1` prints
...
`%struct.nish_result.<T>.<E> = type { i1, <T>, <E> }`, held by pointer and
...
need function values, which Nish forbids because the whole-program pass
...
conversion — Rust's `From<E>` needs a trait and Nish has none — so a
...
[`runtime/nish.d.ts`](../runtime/nish.d.ts) on the include path it
...
`nish` remains the authority: `tsc` cannot see that a `Result` may not be
...
- **The initialiser is an ordinary Nish expression restricted to literals
...
- It takes no parameters (`` `main` cannot take parameters ``,
  ...
  *(CLI only)*.
- The user's function is emitted as `@nish_main`; the compiler adds a C
  `@main(i32 %argc, i8** %argv)` wrapper that builds `process.argv` when the
  program reads it (`call void @nish_argv_init(i32 %argc, i8** %argv)`,
...
(`Condition must be boolean, got i32 (Nish has no truthiness)`,
...
`nish_parse_number(s, mode)` in the runtime (mode 0 `parseFloat`, 1 `Number`,
...
| Signature | Semantics | Effect | Test |
| --- | --- | --- | --- |
| `process.exit(code: i32): void` | `nish_exit` -> libc `exit(code)` (stdio buffers of linked C code are flushed; the arena is abandoned); statement position; a terminator | write, `noreturn` | `process_exit`, `reject_exit_unreachable` |
...
the `@main` wrapper builds once with `nish_argv_init(argc, argv)` before the
...
parameter `const nish_array *` when the whole-program fixpoint proves nothing
...
int32_t sum(const nish_array *xs);
...
- **A module's own `Map` or `Set` wins in that module.** A module that
  ...
  though: as for any class name in scope, a module that also uses the global
  `Map` refuses a type parameter of that name with
  `` `Map` is generic: it must be written with its type arguments ``
  (`tests/cases/reject_map_type_param_beside_global`), where `tsc` would
  shadow it. A class name is program-wide, though, so a
...
- **A program can be run under Node** as the TypeScript it is, in f64 mode and
  within a stated overlap, with `runtime/nish.mjs` supplying the globals
...
3. **A reclaim at the call site.** A function that *returns* a string can have
   ...
   `nish_arena_keep(mark, s)`, which moves the returned string down onto the
...
4. **Explicit control** with the [`Arena`](#arena) builtins, and
   `nish_reset_arena()` / `nish_arena_mark()` / `nish_arena_release()` for a C
...
- **Performance warnings are the one diagnostic that is not an error**
  ...
  - **a function kept external by `--no-strict-exports`** — a call, inside a
    ...
    (`tests/cases/perf_inline`). What it costs is measured as a **size**: 240
    bytes on `bench/sieve`, where `opt -O3` deletes the out-of-line copy under
    the default and keeps it under the flag, and nothing at all on
    `bench/spectral`, whose binary is in fact 64 bytes *smaller* with it. The
    wall clock does not move on either
    ([wp15-performance.md](wp15-performance.md) §9), so the message names the
...
| --- | --- | --- |
| `any` anywhere, including nested type arguments | `` `any` is forbidden in Nish `` | `reject_any_param`, `reject_any_nested` |
| `unknown` | `` `unknown` is forbidden in Nish `` | `reject_unknown_param`, `reject_unknown_nested` |
| `symbol` type | `` `symbol` type is forbidden in Nish (no symbol type) `` | `reject_symbol_type` |
| `bigint` type | `` `bigint` type is forbidden in Nish (use number, i32, or f64) `` | `reject_bigint_type` |
| `undefined` type | `` `undefined` is forbidden in Nish; use `null` with a `T \| null` type `` | `reject_union_undefined` (via the union rule) |
...
| Construct | Message | Test |
| --- | --- | --- |
| `with` | `` `with` is forbidden in Nish (no dynamic scope) `` | `reject_with_statement` |
| `try` / `catch` / `finally` | `` `try`/`catch`/`finally` is forbidden in Nish (no unwinding; use `Result<T, E>`) `` | `reject_try_catch` |
...
| Construct | Message | Test |
| --- | --- | --- |
| `eval(...)`, `eval` as a value | `` `eval` is forbidden in Nish (no interpreter at runtime) `` | `reject_eval` |
| `Function(...)` | `` `Function` constructor is forbidden in Nish (no interpreter at runtime) `` | (validator; no case) |
| `new Function(...)` | `` `new Function` is forbidden in Nish (no interpreter at runtime) `` | `reject_new_function` |
| `Function` as a value | `` `Function` is forbidden in Nish (no interpreter at runtime) `` | (validator; no case) |
| `new Proxy(...)` | `` `new Proxy` is forbidden in Nish (no dynamic property interception) `` | `reject_proxy` |
| `Proxy` as a value | `` `Proxy` is forbidden in Nish (no dynamic property interception) `` | (validator; no case) |
| `Reflect` | `` `Reflect` is forbidden in Nish (no runtime reflection) `` | `reject_reflect` |
...
| `Object.assign/create/defineProperty/defineProperties/setPrototypeOf/getPrototypeOf` | `` `Object.assign` is forbidden in Nish (object layout is fixed at compile time) `` (and analogous) | `reject_object_assign`, `reject_object_define_property`, `reject_object_set_prototype` |
...
parameter message, the `%.17g` comment in `nish.h`, the exclusive
