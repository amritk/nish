...
**The checker rejects it** (`NL2280`): an element reference may not be held
...
`ps.push(ps[i])` has a message of its own (`NL2281`) — the argument is read
...
7. **Contiguous record arrays** (§2a) — **done for `interface` elements**.
   ...
   The layout, the escape rule (`NL2280`/`NL2281`) that makes the dangling
