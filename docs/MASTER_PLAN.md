...
| M6 "One compiler" | WP19 ([wp19-stage0-retirement.md](wp19-stage0-retirement.md)) | stage0 is deleted rather than frozen. The six gates of §3 there are closed first: parity, oracle succession, the seed protocol, the seed policy, distribution without Node, and the provenance tag. | open — after M4. **The first gate, parity, is green as of 2026-09-18**, and this row deliberately does not carry the count: the number has a shelf life of about a day on this repository because the corpus keeps growing (wp19 §A7), and this gate has gone from green to red on a single night four times. A count repeated here is a count that will be wrong before anybody reads it. The measurement with its date is R1's row in [wp19 §5](wp19-stage0-retirement.md#5-order), and the live answer is the nightly `Parity` workflow, which opens an issue on a red or unmeasured full run and closes it on a green one |
