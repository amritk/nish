...
import { jobsFrom, pool, run } from "../tests/pool.js";
 ...
 * from `codeFor`.
 */
async function codesOf(entry) {
  const r = await run(process.execPath, [cli, path.relative(root, entry.file), "--json", ...entry.args], {
    cwd: root,
  });
  const found = new Set();
  for (const match of `${r.stdout}${r.stderr}`.matchAll(/"code"\s*:\s*"(NL\d+)"/g)) found.add(match[1]);
  return found;
}

/**
 * Run the two tables above and answer the codes whose words came out, naming
 * any that did not. A missing wording is a failure of the table rather than of
 * the compiler: the command still printed something, and what it printed is no
 * longer the rule the table says it is.
 */
function harnessPinnedCodes(byCode) {
...
/**
 * The backlog file, as `code -> note`. A line is
 *
 *   NL2019  # <the rule's fragment>  ## <why nothing reaches it>
 *
 * where the `#` half is regenerated and the `##` half is written by hand and
 * kept. Some entries are work — a case nobody has written yet — and some are
 * rules no program can reach, usually because another rule fires first; the
 * note is the difference, and it is why this file is a document rather than a
 * list of numbers.
 */
function backlog() {
  const notes = new Map();
  if (!fs.existsSync(BACKLOG)) return notes;
  for (const line of fs.readFileSync(BACKLOG, "utf8").split("\n")) {
    const text = line.trim();
    if (text.length === 0 || text.startsWith("#")) continue;
    const code = text.split(/\s/)[0];
    const at = text.indexOf("##");
    notes.set(code, at >= 0 ? text.slice(at + 2).trim() : "");
  }
  return notes;
}

function writeBacklog(missing, byCode, notes) {
...
#
# GENERATED by scripts/check-diagnostic-coverage.mjs --update, except for the
# notes: everything after \`##\` on a line is written by hand and kept across a
# regeneration. A note says *why* nothing reaches the rule, and it is what
# separates the two kinds of entry here — a case nobody has written yet, and a
# rule no program can reach because another one fires first.
#
# The check is a ratchet: a code that leaves this file may not come back, and a
# code that is not here and not covered fails the run. Write a \`reject_*\`
# case, run the script with --update, and the line disappears.
#
# ${missing.length} live codes, measured against every \`reject_*\` case, the
# \`tests/link/\` negatives, the driver command lines and the programs that warn.
# Retired and shadowed rules are not here and never will be: no message
# contains their words, or a longer fragment always wins, so no case can reach
# them either way.
`;
  const lines = missing.map((code) => {
    const note = notes.get(code);
    const kept = note === undefined || note.length === 0 ? "" : `  ## ${note}`;
    return `${code}  # ${byCode.get(code) ?? ""}${kept}`.trimEnd();
  });
...
async function main(argv) {
  ...
  const driver = harnessPinnedCodes(byCode);
  ...
  // behind it.
  const closed = [...known.keys()].filter((code) => !missing.includes(code)).sort();
  const unlisted = missing.filter((code) => !known.has(code));

  if (argv.includes("--update")) {
    writeBacklog(missing, byCode, known);
