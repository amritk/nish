 ...
 *   AS9xxx         WP15 section 8 performance warnings
 *   AS0000         no rule matched: a diagnostic that has no code yet
 *
...
const collect = () => {
  ...
  for (const abs of sources(path.join(ROOT, "src"))) {
    ...
    for (const m of [...text.matchAll(call), ...text.matchAll(table)]) {
      ...
      // Such a diagnostic stays AS0000 until its wording grows something to key
...
const build = () => {
  ...
  for (const e of entries) {
    ...
    e.code = `AS${e.band}${String(next).padStart(3, "0")}`;
  }
...
const stage1Source = (entries) => {
  ...
  const rows = (list) =>
    list.map((e) => `    ${JSON.stringify(e.fragment)},\n    "${e.code}",`).join("\n");
  // AmritScript has no module-level array constant (a constant's initialiser
