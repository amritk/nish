/**
 * Ambient declarations for the StaticTS global surface.
 *
 * Every StaticTS program is legal TypeScript syntax — the compiler parses it
 ...
 *
 *   { "include": ["src/**\/*.ts", "node_modules/statictsc/runtime/statictsc.d.ts"] }
 *
 * **`statictsc` is still the authority.** TypeScript's structural checker is
 * weaker than this compiler's in the places where StaticTS is deliberately
 * stricter, and it cannot see the flow rules at all:
 *
 *   - the integer and float widths are aliases of `number` here, so `tsc`
 *     lets an `i32` and an `f64` mix where `statictsc` refuses;
 ...
 *
 * So a program that `tsc` accepts may still be rejected by `statictsc`; the
...
//
// StaticTS treats these as distinct types that never mix implicitly. There is
...
// are aliases and the width check is left to `statictsc`.
...
interface ResultMethods<T, E> {
   ...
   * function — Rust's `?`. `statictsc` additionally requires that function to
...
// ---- console and process -----------------------------------------------------
//
// Declared here with *StaticTS's* signatures — one argument, no format string,
...
// ---- Numeric conversions (there is no cast in StaticTS) ----------------------
...
// ---- Streams and files (globals: StaticTS has no package resolution) ---------
...
// `BigInt64Array`) are *not* declared here. In StaticTS each one names the
