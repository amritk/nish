...
| R1 | parity: no program and no flag is stage0's | **done, and re-opened once** — the mode is not part of `npm test`, so it is green as of the last time someone ran it. It was red on `main` at 045c8f8 on two `-g` defects — a function's position taken from the arrow rather than from its declaration, and a `DILocation` column counted in UTF-16 code units rather than bytes — which §A5 of [`wp19`](../docs/wp19-stage0-retirement.md) records; re-run it before citing this row. `node tests/run.js --parity` is **red** as of 2026-09-14 — **13,020 runs over 868 programs, 18 undeclared differences, 2,646 declared** (§A7) — and none of the 18 is this row's own family: stage1 exits 70 under `-g` on a `CPtr` program (WP27) and answers a different diagnostic on `reject_generic_expanding_field` (WP18). Four times now a number has been written down here and been false by the next run, and the corpus growing is the cause every time (§A5, §A7): 764 programs, then 815, then 868 a day later. **Do not cite this row — run the mode.** A count here goes stale in about a day.  G1's check is `tests/self/parity.js`, in two halves: the flag sets each `--help` names, and the corpus under every flag variation. `getenv` and `--emit-ast` landed, the performance warnings and `--no-warn-performance` are stage1's, `--out-dir` is gone, the oracles' stale skips are closed. `--emit-checked` prints the attribute pass's facts now, so `checked_oracle.js` filters nothing, four of §A2's five differences are fixed — the fifth was misrecorded and is really §A3's recovery class — and §A3's five classes are closed too: four in code and one by declaration. Recovery is the one that mattered, and closing it turned up four more contextual-type divergences of the same family, because a compiler that reports every consequence of a mistake buries the ones that are its own |
