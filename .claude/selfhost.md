...
| R1 | parity: no program and no flag is stage0's | **in progress** — G1's check is `tests/self/parity.js`, in two halves: the flag sets each `--help` names, and the corpus under every flag variation. `getenv` and `--emit-ast` landed, the performance warnings and `--no-warn-performance` are stage1's, `--out-dir` is gone, the oracles' stale skips are closed. `--emit-checked` prints the attribute pass's facts now, so `checked_oracle.js` filters nothing, and four of §A2's five differences are fixed — along with three more of the same family that fixing them turned up, among them the contextual type stage1 was letting reach a binary operator's operands. What is left is the five classes §A3 of [`wp19`](../docs/wp19-stage0-retirement.md) names: run over the whole corpus rather than `tests/cases`, the mode reports 13,800 undeclared differences, three of them classes that want a declaration (a path inside the IR, a parser refusal that beats Phase 0 to it, one wording) and two that want code (`--emit-ast` on a refused program, and error recovery after the first diagnostic) |
