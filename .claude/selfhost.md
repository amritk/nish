...
| `tests/self/reject_oracle.js` | every `reject_*` case and every `tests/link/` negative, against its own expected fragments — and its **parser bucket against a ceiling**, [`tests/self/parser_refusals.txt`](../tests/self/parser_refusals.txt), which names each case and the sentence stage1 answers with (89 entries) and is compared in both directions, so a rule leaving the parser's reach is an edit to that file rather than a tally nobody re-read. The comparison itself is driven over fabricated inputs by a `selfCheck` on every run |
| `tests/diagnostic_coverage.js` | each compiler against `tests/wordings/`, one program per diagnostic code, and against **the registry**: a code in `self/codes.ts` that no program provokes and no reason explains fails the run (WP19 G2.4, "The wording gap"). Asked of **stage1** as well as stage0 since 2026-09-21, which is what the gate's criterion says and took a third register, [`tests/wordings/stage0_only.txt`](../tests/wordings/stage0_only.txt): the codes whose programs stage1 answers in its parser, joined to the per-case registers that already said what it answers them with |
...
| `tests/self/parity.js` | everything the command line produces (WP19 G1), in two halves: the flag set each compiler's `--help` names, diffed — the only thing here that asks what flags a compiler *has* — and then the corpus through both compilers under each of the sixteen flag variations the suite uses, comparing exit status, stdout, stderr and every file written. A difference is a failure unless `DECLARED` names it with a reason. **Those sixteen are a list somebody wrote, not a property of the compilers** — `--json` was missing from it until wp19 §A8, and was worth 180 undeclared differences on an otherwise-green corpus |
