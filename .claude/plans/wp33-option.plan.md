...
Pinned to `main` at **e5e0cfe** (#230) and to WP32 S3's PR #232 (`feature/wp32-map/map-get`) at 9bf9650. Line references are those commits'. The plan went through nine adversarial reviews — two on the draft and one on each of seven revisions — until a round found no blockers or majors; each finding it acts on is folded into the section it changes. Four bugs they surfaced in the compiler already on `main` are filed separately: #233, #234, #235, and the lazy `Result.unwrapOr` argument (filed by the S1 note).
...
**Owns:**
- **Compiler:** [self/types.ts](../../self/types.ts), [self/annotations.ts](../../self/annotations.ts), [self/option.ts](../../self/option.ts) (new), [self/emit_option.ts](../../self/emit_option.ts) (new), [self/program.ts](../../self/program.ts) (the `None` side table), [self/expressions.ts](../../self/expressions.ts) (`computeType`, `checkConditional`, `narrow`), [self/members.ts](../../self/members.ts), [self/statements.ts](../../self/statements.ts), [self/emit.ts](../../self/emit.ts), [self/generics.ts](../../self/generics.ts) (`unifyAnnotation`, `containsType`, `originOfMember`), [self/structs.ts](../../self/structs.ts), [self/constants.ts](../../self/constants.ts) and [self/arrays.ts](../../self/arrays.ts) (the gate, and the array-literal canonicalisation), [self/debug.ts](../../self/debug.ts), [self/interop_abi.ts](../../self/interop_abi.ts) (explicit `""` from `cType`/`tsKeyword`), [self/codes.ts](../../self/codes.ts) (and `RULE_COUNT`).
...
| Piece | Where | What |
| --- | --- | --- |
| kind and interning | types.ts `internAll` :274, `assignable` :644 | key `K_OPTION` by state (`O_UNKNOWN`/`O_SOME`/`O_NONE`, reusing `states[t]`); `alignOf` answers the payload's alignment; **`assignable` ignores the `Option` state, as its `K_RESULT` branch does** — narrowing re-interns a local under a new id (`scope.narrow(local, table.withState(…))`, result.ts :376), so without it `if (o.isNone()) return None; return o;` and `f(o)` after `o.isSome()` would be refused. `option_narrow` pins both. **A proof state is visible only to a proof consumer** — the receiver of `some`/`value`/`isSome`/`isNone`/`unwrapOr`/`expect`/`orReturn`/`okOr`, and a narrowing test. Everywhere else a value flows — the *flowing type* is canonicalised at the flow site, not only when the node is an identifier, so `const y = (o)` is covered too — (a `let` or `const` initialiser, a ternary arm, an array element, `Some(x)`, an argument, a return, a store) its type is canonicalised to `O_UNKNOWN`, and `optionOf` canonicalises a payload `Option` the same way. Because `assignable` is state-blind, any narrower rule leaks: rounds 6 and 7 found the proof escaping through a `let` (statements.ts :207), a nested `Some(x)` payload, a ternary (`checkConditional` returns `whenTrue`, expressions.ts ~:849–873) and an array literal (`checkArrayLiteral`'s element type, arrays.ts :35–66). Each lets `.value` read an unproven payload — `0` natively, `undefined` under Node — where `tsc` refuses. `Result` has the ternary and array leaks today, reproduced on the 0.10.0 seed (#234; the `let` one is masked by #233). The cost is the safe direction: `const y = x` inside a narrowing does not carry the proof, so `y.value` is refused where `tsc` accepts it, which LANGUAGE.md documents. Pinned by `reject_option_let_keeps_proof`, `_nested_keeps_proof`, `_ternary_keeps_proof` and `_array_keeps_proof` (in S2 the array case is refused first by `_element_inferred`; S4b, which lifts that gate, re-pins it on the proof rule and updates its `.err`); `option_narrow` keeps the early-return and direct-receiver forms accepted. |
...
- The `Result` follow-ups the note files: eager `unwrapOr`/`expect` and contextual `Ok` payloads. The `Result` proof-state leaks are live bugs, filed as #234 (ternary, array literal) and on #233 (`let`), and are fixed on their own, not here.
