...
| Piece | Where | What |
| --- | --- | --- |
| kind and interning | types.ts `internAll` :274 | key `K_OPTION` by state (`O_UNKNOWN`/`O_SOME`/`O_NONE`, reusing `states[t]`); `alignOf` answers the payload's alignment |
...
| an instantiation of an exported template that is not a library copy, at an `Option` argument | `_export_instance` | S5 |
...
**Host-visible** (the `_export` and `_export_instance` rows) means an exported function or a method of an exported class — what `cType` bridges — decided by **the same rule as Q14: `exported && !definedIn(library)`**. The checker can decide it (program.ts :1219 on the map-get branch records whether a module is `std/collections.ts`), so `Map`'s own methods, instantiated at an `Option` value in S4b, are never refused by either row.
