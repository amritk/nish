...
**Tests:**
- `gen_option_first`: `first<T>(xs)` at `i32`, `f64`, `bool`, `string`, a class, an interface, `Node | null` and `Option<i32>`.
...
If Q14 leaves the 8-byte question open, this stage becomes its own WP, as WP17 followed WP16.
...
10. Exported `Option<i32>`, `Option<f64>` and `Option<Node>` functions round-trip through C, N-API and wasm, and a `--no-strict-exports` build compiles them (S5).
