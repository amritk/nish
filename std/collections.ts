 ...
 *
 * A program sees only the JavaScript members (§7): `size`, `get`, `set`,
 * `has`, `delete` and `clear` on a `Map`, and `size`, `add`, `has`, `delete`
 * and `clear` on a `Set`. Everything else in this file is refused by name
 * outside it, as if it did not exist — which under `tsc` it does not. `get`
 * is the one member with no method here: it answers `V | undefined`, which
 * never crosses a call, so the compiler lowers `m.get(k)` in place to a
 * `probe` and, on the edge where it found the key, a `valueAt` (§3.2).
 */
