  ...
  rejectSymbolClashes(): void {
    ...
    for (const unit of this.modules) {
      ...
      for (const template of unit.checker.program.templateList) {
        ...
        // One template literal rather than a `+` after "; a function ": the
        // code generator keys a rule on the longest literal run of a message,
        // and split there the longest run was "` is also defined in ", which
        // the three whole-program duplicate messages also contain.
        this.sink.report(
          unit.source,
